...
from utils.evaluations import do_prc_roc
...
def create_logdir(method, weight, rd):
    ...
    return "bigan/train_logs/sia/{}/{}/{}".format(weight, method, rd)
...
def run(nb_epochs, w, method, degree, label, random_seed=42):

    """ Runs the training process"""

    # for w in np.arange(11)*0.1:
    with tf.Graph().as_default():
        # Set the graph level seed
        #          tf.set_random_seed(random_seed)
        train_and_test(nb_epochs, w, method, degree, random_seed)
